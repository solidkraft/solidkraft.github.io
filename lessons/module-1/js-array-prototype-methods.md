...
```javascript
var students = ["Scott", "Heather", "Jeremiah"];
```

For the array above,
- Oops! We're missing an student in this array. We need to add "Sole" to the list. What are the **instructions** you would want to tell the computer to take in order to add a new student to the array? 
- I need to know if a certain student named "Heather" is in the students array. What are the **instructions** you would want to tell the computer to take in order to find out if "Heather" is included in the array?
...
var studentExists = students.includes('Heather');

console.log(studentExists); // logs true
```

### Mutator Methods

In other cases, we will want to change the data our array holds. **Mutator** methods actually mutate, or change, the data in the array. If we needed to remove an student element from the `students` array, we'd need to use a mutator method.
...
console.log(students) // logs ["Scott", "Heather", "Jeremiah", "Sole"];
```
<section class="call-to-action">
...
3. Visually represent the method using sticky notes, drawings in your notebook, or using an online tool like [Jamboard](https://jamboard.google.com/){:target="_blank"}
...
### Practice
Work through the exercises in <a target="_blank" href="https://replit.com/@danpariente/js-prototype-array-methods-practice#index.js">this REPL</a>.
</section>
